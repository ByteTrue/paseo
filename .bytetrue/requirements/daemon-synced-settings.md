...
把跟 daemon 工作能力直接相关的三件事搬到 daemon 自己身上持久化：主机显示名、创建 agent 时的模型偏好和收藏、供应商可用模型的上一份探测结果。新浏览器连上来时，daemon 直接告诉它这些信息——显示名、偏好、模型列表——秒开可用。探测供应商的事后台跑，旧结果先顶上，新的出来了再更新。
