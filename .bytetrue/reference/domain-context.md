...
- **Daemon administrator password** — Shared secret for enrolling remote clients and authorizing auth administration. UI: "Password" in daemon auth contexts. Don't call it a "pairing password"; it is not embedded in pairing links.
- **Auth administration** — Managing daemon authorization itself: setting/changing the daemon administrator password, enrolling authorized clients, and revoking authorized clients. Requires the daemon administrator password once one exists; don't infer it from localhost, terminal, environment variables, or CLI origin.
- **Authorized client** — A client identity that a daemon has explicitly allowed to control it. Use when authorization status matters; don't use "connected client" as a synonym.
- **Client auth keypair** — Per-daemon/host Ed25519 signing keypair stored by a client and enrolled by a daemon. Keyed by `serverId`; don't reuse one client auth keypair across daemons.
- **Pairing link** — URL or QR code used to locate a daemon through the relay and verify the daemon identity. Treat as sensitive bootstrap material, but don't describe it as a complete authorization grant.
