...
```bash
python .bytetrue/tools/search-yaml.py --dir {目录} [--filter key=value]... [--query "全文关键词"] [--sort-by FIELD [--order asc|desc]] [--full] [--json]
```
...
沉淀类文档统一在 `.bytetrue/compound/`，用 `doc_type` 字段区分四个子技能的产物，内部还有各自的细分字段：
...
| 场景                            | 命令建议                                                                                                                                               |
| ------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------ |
| feature-design 开始前查已有归档 | 搜 `.bytetrue/compound` 目录，按 `--query "{关键词}"` 全文搜；要分类看就加 `--filter "doc_type=learning\|trick\|decision\|explore"`                    |
| issue-analyze 根因分析前查历史  | 搜 `.bytetrue/compound` `--filter doc_type=learning --filter track=pitfall`、再搜 `--filter doc_type=trick --filter type=library`，按相关组件/框架过滤 |
...
```bash
# 校验单个文件的 YAML 语法
python .bytetrue/tools/validate-yaml.py --file {文件路径} --yaml-only

# 校验必填字段
python .bytetrue/tools/validate-yaml.py --file {文件路径} --require doc_type --require status

# 批量校验目录下所有文件
python .bytetrue/tools/validate-yaml.py --dir {目录} --require doc_type --require status
```
