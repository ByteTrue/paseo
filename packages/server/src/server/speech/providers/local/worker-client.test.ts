...
describe("LocalSpeechWorkerClient", () => {
  ...
  it("does not surface real IPC backpressure when replaying native-sized dictation frames", async () => {
    ...
    try {
      ...
      const observedMessage = observedError instanceof Error ? observedError.message : null;
      expect(observedMessage).not.toBe("Local speech worker IPC channel is not writable");
    } finally {
      client.shutdown();
