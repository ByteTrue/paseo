...
export async function generateLocalPairingOffer(args: {
  ...
  const relayEndpoint = args.relayEndpoint ?? "relay.paseo.zijieapi.de5.net:443";
  const relayPublicEndpoint = args.relayPublicEndpoint ?? relayEndpoint;
  const relayUseTls = args.relayUseTls ?? relayEndpoint === "relay.paseo.zijieapi.de5.net:443";
  const relayPublicUseTls = args.relayPublicUseTls ?? relayUseTls;
  const appBaseUrl = args.appBaseUrl ?? "https://paseo.zijieapi.de5.net";
