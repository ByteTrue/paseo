...
> Web, desktop, and CLI app for monitoring and controlling your local AI coding agents from anywhere. Your dev environment, in your pocket.
...
A self-hosted daemon manages agent lifecycle, exposes a WebSocket API, and ships with an MCP server so other agents can talk to it. Desktop apps for macOS, Windows, and Linux, a browser web app that works on phones as a PWA, and a Docker-style CLI ("paseo run", "paseo ls", "paseo logs", "paseo wait") let you launch sessions, watch them work, review diffs, and ship from anywhere. An end-to-end encrypted relay lets browser clients reach your daemon over the public internet without exposing it.
...
Distribution: desktop apps for Mac, Windows, and Linux; web app/PWA; Homebrew; npm. Source: AGPL-3.0 at https://github.com/ByteTrue/paseo. Marketing site: https://paseo-website.bytetrue.workers.dev.
`;
...
- [Changelog](${SITE_URL}/changelog): Release notes for the Paseo daemon, CLI, desktop, and web apps.
- [Download](${SITE_URL}/download): Install Paseo on Mac, Windows, Linux, or run the web app.
