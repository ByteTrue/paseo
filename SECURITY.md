...
The relay is designed to be untrusted. All relay traffic between your client and daemon is end-to-end encrypted. The relay server cannot read your messages, see your code, or modify traffic without detection. Even if the relay is compromised, your data remains protected.
...
3. When a browser/PWA client connects via the relay, it generates a fresh ephemeral Curve25519 keypair and sends an `e2ee_hello` message containing its public key. The daemon will not process any application messages until this handshake completes.
...
- **Send commands as you** — The daemon only accepts traffic that decrypts and authenticates under a shared key derived with its own secret key. The client's keypair is ephemeral per connection, so there is no persistent phone-side secret to steal; protection comes from the daemon's secret key never leaving the daemon.
...
The QR code or pairing link carries the daemon's public key and relay/discovery metadata. It is a long-lived bootstrap URL, not a complete authorization grant: remote clients still need to enroll with the daemon administrator password before receiving daemon data. Treat pairing links as sensitive because they reveal how to reach the daemon and allow enrollment attempts, but password verification and rate limiting are the authorization boundary.
...
The daemon supports a daemon administrator password (set via `auth.password` in `config.json` or the daemon-side `PASEO_PASSWORD` env var; stored bcrypt-hashed). The password is not embedded in pairing links and is not the normal reconnect credential for enrolled remote clients. Direct TCP URL password parameters, client-side `PASEO_PASSWORD`, and stored host `password` fields are not connection grants in the redesigned auth model; legacy parsers may accept and discard them for compatibility. Instead, remote clients enroll with the administrator password once, receive no daemon data until enrollment succeeds, and then prove authorization on each connection with their enrolled client signing key. Auth administration actions such as enrolling, revoking clients, or changing the administrator password require the administrator password every time. Administrator-password submission is allowed only over relay E2EE, IPC/localhost, or TLS direct TCP; non-loopback cleartext direct TCP must reject enrollment and auth administration. Remote administrator-password failures are rate-limited by remote source and client public key with exponential backoff; localhost/IPC remains lenient and primarily logs failures.
